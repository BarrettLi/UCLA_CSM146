...
class PolynomialRegression() :
    ...
    def fit_GD(self, X, y, eta=None,
        ...
        for t in xrange(tmax) :
            ...
            gradient = 0
            for i in range(n):
                gradient += (np.dot(X[i], self.coef_) - y[i]) * X[i].transpose()
            ...
            if t > 0 and abs(err_list[t] - err_list[t-1]) <= eps :
                print "Theta is:"
                print self.coef_
                print "value of objective funtion:"
    ...
    def cost(self, X, y) :
        ...
        for i in range(len(X)):
            cost += (np.dot(X[i],self.coef_) - y[i])**2
...
def main() :
    ...
    # fig, ax = plt.subplots()
    # ax.plot(a, training_error, "r", label="traing_error")
    # ax.plot(a, testing_error, "b", label="testing_error")
    # plt.legend()
    # plt.show()
    ### ========== TODO : END ========== ###
